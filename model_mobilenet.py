"""
MobileNetV2 model implementation with ImageNet pretrained backbone
...
class MobileNetV2BinaryClassifier(nn.Module):
    """
    MobileNetV2 architecture with ImageNet pretrained backbone
    ...
    def __init__(self, pretrained: bool = True, num_classes: int = 2, dropout: float = 0.5):
        ...
        """
        super(MobileNetV2BinaryClassifier, self).__init__()
        ...
        # MobileNetV2's classifier is at mobilenet.classifier
        # The last channel number is stored in last_channel
    ...
    def freeze_backbone(self):
        ...
        print("MobileNetV2 backbone frozen. Only classifier will be trained.")
    ...
    def unfreeze_backbone(self):
        ...
        print("MobileNetV2 backbone unfrozen. All layers will be trained.")
...
def create_mobilenet_model(pretrained: bool = True, 
                          num_classes: int = 2,
                          dropout: float = 0.5,
                          freeze_backbone: bool = False,
                          device: Optional[torch.device] = None) -> MobileNetV2BinaryClassifier:
    """
    Factory function to create and configure MobileNetV2 model
    ...
    Returns:
        Configured MobileNetV2 model
    """
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    model = MobileNetV2BinaryClassifier(
