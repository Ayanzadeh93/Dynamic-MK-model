...
from model_resnet import create_resnet18_model, get_optimizer, get_scheduler
...
class Trainer:
    """
    Trainer for ResNet-18 with Train/Test split
    """
    ...
    def train_model(self, train_paths, train_labels, test_paths, test_labels) -> Tuple[Dict, Dict]:
        ...
        metrics_calculator = BinaryClassificationMetrics(save_dir=self.results_dir)
...
def main():
    ...
    config = {
        ...
        # Model
        'pretrained': True,
        'freeze_backbone': False,  # Train ALL layers (all weights trainable) - matches high-performance setup
        
        # Training - Matching high-performance ResNet50 settings
        'num_epochs': 100,  # More epochs for better convergence
        'batch_size': 16,
        'learning_rate': 3e-4,  # Lower LR for full fine-tuning (all layers trainable)
        'weight_decay': 0.05,  # Higher weight decay for regularization
        'optimizer': 'adamw',  # AdamW optimizer (better than Adam for this setup)
        ...
        # Results
        'results_dir': 'final_results/resnet18'
    }
